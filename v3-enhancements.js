...
async function v3FetchOriginalBlob(photo) {
  ...
  if (isAuthed()) {
    try {
      ...
      if (id) {
        const r = await api(`/api/photos/${encodeURIComponent(id)}/file`, {raw: true});
        if (r instanceof Blob) return r;
        if (r && typeof r.blob === 'function') return await r.blob();
      }
    } catch (_) {}
  }
  return local;
