...
assert.match(history, /openServerHistory/);
assert.match(history, /data-mark-all/);
assert.match(history, /async function markAllSeen/);
assert.match(history, /v340MarkSpreadSeen/);
assert.match(worker, /on\('PUT', '\/api\/spreads\/:id\/activity\/seen'/);
assert.match(worker, /unread\.spreads\[row\.spread_id\]/);
assert.match(read('backend/migrations/0003_activity_spread_seen.sql'), /CREATE TABLE activity_spread_seen/);
assert.match(core, /v340MigrateLegacyCovers/);
assert.match(core, /role !== 'OWNER'/);
assert.match(core, /cover_migration:'member-skip'/);
